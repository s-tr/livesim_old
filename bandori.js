...
function __drawLNBandori(curTime,ln){
	var visBegin=0;
	...
	if(visEnd<visBegin){
		var note1=ln.notes[visEnd];
		var note2=ln.notes[visBegin];
		var firePos=note1.lane+(note2.lane-note1.lane)*(curTime-note1.time)/(note2.time-note1.time);
		__drawLNConnectBandori.call(this,curTime,
				curTime,firePos,
				curTime+this.approachTime,note2.lane);
		this.drawNote(curTime,new Note(curTime,firePos,Note.LN_BEGIN));
		return;
	} else if(visEnd==visBegin){
	} else {
	...
	if(visBegin!=0){
		var invisNote=ln.notes[visBegin-1];
		var visNote  =ln.notes[visBegin];
		var firePos  =invisNote.lane+(visNote.lane-invisNote.lane)*(curTime-invisNote.time)/(visNote.time-invisNote.time);
		__drawLNConnectBandori.call(this,curTime,
		curTime,firePos,
		visNote.time,visNote.lane);
		this.drawNote(curTime,new Note(curTime,firePos,Note.LN_BEGIN));
	}
	
	if(visEnd!=ln.notes.length-1){
		var invisNote=ln.notes[visEnd+1];
		var visNote  =ln.notes[visEnd];
		__drawLNConnectBandori.call(this,curTime,
		visNote.time,visNote.lane,
		curTime+this.approachTime,invisNote.lane);
	}
